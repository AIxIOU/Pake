(function() {
    ...
    let sidebarEl = null;   // set once the sidebar exists
    let sizeObserver = null;

    function sidebarWidth() {
        return collapsed ? SIDEBAR_W_COLLAPSED : SIDEBAR_W;
    }
    ...
    if (!document.getElementById('gbf-simple-sidebar')) {
        ...
        function applyCollapsed() {
            sidebar.classList.toggle('gbf-collapsed', collapsed);
            toggle.textContent = collapsed ? '\u00AB' : '\u00BB';
            document.querySelectorAll('.gbf-section-title').forEach(t => {
                if (!t.dataset.full) t.dataset.full = t.textContent;
                t.textContent = collapsed ? t.dataset.full.charAt(0) : t.dataset.full;
            });
            applyBodyInset();
        }
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === '\\') { toggleCollapsed(); e.preventDefault(); return; }
            if (key === 'l') { toggleLocked(); e.preventDefault(); return; }
            if (key === 'r') { location.reload(); e.preventDefault(); return; }
            if (e.key === 'ArrowLeft') { history.back(); e.preventDefault(); return; }

            const item = NAV.filter(i => i.key).find(i => i.key === key);
            if (item) { go(item.hash); e.preventDefault(); }
        });
    ...
    function positionSidebar() {
        ...
        if (window.localStorage && localStorage.getItem('gbfDebug') === '1') {
            console.log('[gbf-lock] game right edge=' + edge + ' -> outer left=' + Math.round(edge));
        }
    }
